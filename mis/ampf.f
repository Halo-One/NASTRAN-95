...
C     K) PAIR, 0 WHEN NOT KNOWN (AMPC, AMPD, AMPF)
...
C
C     HALO: AS IN AMPD - REOPENED WHERE THE LAST PAIR LEFT IT WHEN THAT
C     IS COLUMN AJJCOL, INSTEAD OF REWOUND AND WALKED AJJCOL-1 COLUMNS
...
C     POSITION AJJL
C
C     HALO: AS IN AMPC - NOT REWOUND AND WALKED WHEN THE LAST PAIR'S
C     COPY LEFT AJJL AT COLUMN AJJCOL
      IF (NEXTC.GT.0 .AND. AJJCOL.EQ.NEXTC) GO TO 107
...
C     DECOMPOSE AJJ(K)
C
C     HALO: ONE GROUP AND THE MATRICES FIT IN CORE - SOLVE THERE, AS
C     AMPC DOES (AMPCZN: AMPCZ WITHOUT THE TRANSPOSE, RJH = AJJ**-1 S),
C     AND SKIP THE OUT-OF-CORE DECOMPOSITION, HOURS PER MACH ON A LARGE
C     MODEL. N95_INCORE_AJJ=0 KEEPS THE ORIGINAL PATH.
C
      IF (IONCE .EQ. 0) GO TO 1005
      MCB(1) = SCR1
      CALL RDTRL (MCB)
      IAJJT  = MCB(5)
      MCB(1) = SCR3
      ...
      CALL AMPCZN (SCR1,SCR3,RJH,NCOLTH,NOH,ITOUT,MCB(4),ISUCCS)
      IF (ISUCCS .NE. 0) GO TO 1020
 1005 IB = 0
