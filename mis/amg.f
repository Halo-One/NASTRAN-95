...
C
   ...
   90 CALL REWIND (ACPT)
      ...
      IFILE   = ACPT
C
C     HALO: ONE DOUBLET LATTICE GROUP (ONE ACPT RECORD, METHOD 1) - THE
C     PAIRS OF EACH MACH IN BATCHES, EVERY ELEMENT COMPUTED ONCE FOR ALL
C     THE BATCH'S FREQUENCIES (AMGK, MIS/AMGK.F); THE SAME AJJL AND SKJ
C
      IDONE = 0
      IF (IZ(1).EQ.1 .AND. IZ(2).EQ.1)
     1    CALL AMGK (AERO,ACPT,AJJL,SKJ,IZ(3),IDONE,IERR)
      IF (IERR .EQ. 1) GO TO 400
      IF (IERR .EQ. 2) GO TO 420
      IF (IDONE .NE. 0) GO TO 210
C
