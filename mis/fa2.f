...
C
      ...
      CALL READ (*910,*920,FSAVE,IZ(1),9,1,IFLAG)
      PKNL = IZ(9) .EQ. 1
      J     = 3
      FMETH = IZ(J)
      METH  = ME(FMETH)
...
C
      ...
      IMR1  = IMR + 3*NRHO
C
C     HALO: PKNL - THE MATCHED POINTS (EACH ITS OWN DENSITY, MACH AND
C     VELOCITY) ARE ONE GROUP, SO EACH ROOT PRINTS ONE BLOCK WITH A ROW
C     PER POINT, THE LAYOUT MSC PRINTS AND read_nastran_flutter READS
C
      IF (.NOT. PKNL) GO TO 3020
      NCOPY = (IFLAG - IMR + 1)/3
      IMR1  = IMR + 3*NCOPY
      GO TO 1042
 3020 CONTINUE
...
C
      ...
      NLINES = NLINES + 7
      IPT = FLOOP
      IF (FMETH .EQ. 3) IPT = NV
C
C     HALO: A MATCHED POINT BLOCK HAS ITS DENSITY AND MACH IN EVERY ROW,
C     SO ITS HEADER NAMES NEITHER (MSC'S PKNL HEADER); THE OTHERS NAME
C     THE GROUP'S
C
      IF (PKNL) GO TO 1038
      IF (PRINT(1) .EQ. YESB) WRITE (NOUT,1039) IPT,XMACH,RHO,METH
      IF (PRINT(1) .EQ. YES ) WRITE (NOUT,1040) IPT,XMACH,RHO,METH
      GO TO 1041
 1038 WRITE (NOUT,1043) IPT,METH
      GO TO 1041
 1043 FORMAT (1H0,55X,16HFLUTTER  SUMMARY, //7X,
     1        9HPOINT =  ,I3, 5X,9HMETHOD = ,A4,2X,16H(MATCHED POINTS),
     2        ///7X,5HKFREQ, 7X,8H1./KFREQ, 6X,7HDENSITY, 6X,
     3        8HMACH NO., 6X,8HVELOCITY, 7X,7HDAMPING, 6X,
     4        9HFREQUENCY, 6X,18HCOMPLEX EIGENVALUE)
 1039 FORMAT (1H0,55X,16HFLUTTER  SUMMARY, //7X,
     ...
     3        5HKFREQ, 7X,8H1./KFREQ, 6X,7HDENSITY, 6X,8HMACH NO., 6X,
     4        8HVELOCITY, 7X,7HDAMPING, 6X,9HFREQUENCY, 6X,
     5        18HCOMPLEX EIGENVALUE)
...
C     HALO: MSC'S LAYOUT, DENSITY RATIO AND MACH IN EVERY ROW (THE
C     ROW'S OWN FOR PK, WHOSE LOOP TABLE HOLDS MACH, VELOCITY, DENSITY)
      RHOR = RHO
      XMR  = XMACH
      IF (FMETH .NE. 3) GO TO 1065
      RHOR = Z(IMR+3*I-1)
      XMR  = Z(IMR+3*I-3)
 1065 WRITE  (NOUT,1070) KFREQ,ONEOK,RHOR,XMR,VOUT,G,F,REL,IML
 1070 FORMAT (1H ,1X,F9.4,8(1X,1P,E13.6))
 1050 CONTINUE
C
C     PUT OUT OVG PARTS
C
