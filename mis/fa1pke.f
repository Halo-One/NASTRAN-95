...
C
      ...
      CALL SSWTCH (39,L39)
C
C     HALO: N95_PK_THREADS, THE THREADS OF THE PARALLEL PK SOLVE BELOW:
C     1 KEEPS THE SERIAL SOLVE, 0 OR UNSET TAKES THE OPENMP DEFAULT
C     (OMP_NUM_THREADS, WHICH THE SOL 145 DRIVER SETS FOR ITS CHILDREN)
C
      ENVPK = ' '
      CALL GETENV ('N95_PK_THREADS',ENVPK)
      NPKTH = 0
      IF (ENVPK .NE. ' ') READ (ENVPK,*,IOSTAT=IOSPK) NPKTH
      IF (ENVPK .NE. ' ' .AND. IOSPK .NE. 0) NPKTH = 0
      TRL(1) = KHH
      CALL RDTRL (TRL)
      NROW  = TRL(2)
      NEIW  = MIN0(NEIW,NROW)
...
C
C     HALO: THE LOOPS OF THIS MACH ARE SOLVED SIDE BY SIDE ON THE FIRST
C     OF THEM (FA1PKP, MIS/FA1PKP.F: THIS ROUTINE'S ARITHMETIC FROM HERE
C     TO LABEL 300, ONE THREAD PER LOOP) AND EACH LOOP IS WRITTEN HERE
C     IN TURN AS THE SERIAL SOLVE BELOW WOULD HAVE WRITTEN IT (FA1PKR):
C     THE PK EIGENVECTORS AND WARNINGS PRINTED, SCRATCH 301 AND 302, THE
C     ROOTS AT Z(IEIGNS+1). THE CORE CHECKS ARE THE SERIAL SOLVE'S. DIAG
C     39 (THE TRACE), N95_PK_THREADS=1 AND A BASIS OF FEWER THAN FOUR
C     MODES (SEE FA1PKP) KEEP THE SERIAL SOLVE.
C
      IF (L39.NE.0 .OR. NPKTH.EQ.1 .OR. NROW.LT.4) GO TO 107
      I    = (FLOOP-1)*3
      VEL  = ABS(Z(IMVR+I+1))
      NREM = IQ0 - IOP
      ...
      IF (Z(IMVR+I+1).LT.0.0 .AND. IT+NN.GT.BUF1)
     1   CALL MESAGE (-8,0,NAME)
      CALL FA1PKR (FLOOP,NROW,NEIW,BREF,VEL,Z(IEIGNS+1),NROOT,Z(BUF1),
     1             IDONE)
      IF (IDONE .NE. 0) GO TO 104
      CALL FA1PKP (FLOOP,NLOOP,NROW,NK,NEIW,EPS,BREF,RREF,Z(IMVR),
     1             Z(IMH),Z(IBH),Z(IKH),Z(IQ),Z(IA),Z(IK),NPKTH)
      CALL FA1PKR (FLOOP,NROW,NEIW,BREF,VEL,Z(IEIGNS+1),NROOT,Z(BUF1),
     1             IDONE)
      IF (IDONE .EQ. 0) GO TO 107
  104 IF (IDONE .EQ. 2) CALL MESAGE (-61,0,0)
      GO TO 300
  107 CONTINUE
C
      I   = (FLOOP-1)*3
...
C
  ...
  150 CONTINUE
C
C     HALO: THE TWO PRODUCTS AND THE EIGENVALUES THROUGH FA1PKG AND
C     FA1PKQ (MIS/FA1PKQ.F): THE SAME ARITHMETIC AS GMMATS AND FA1PKA
C     (HSBG, ATEIG), THE SAME BITS, COMPILED OPTIMISED - NINE TENTHS OF
C     THE PK LOOP'S TIME ON A LARGE MODAL BASIS. NASA'S PATH IS KEPT
C     FOR DIAG 39, WHICH TRACES FROM INSIDE FA1PKA.
C
      IF (L39 .NE. 0) GO TO 148
      CALL FA1PKG (Z(IB),Z(IMH),Z(IV),NROW)
      CALL FA1PKG (Z(IT),Z(IMH),Z(IB),NROW)
      CALL FA1PKQ (Z(IMA),Z(IV),Z(IB),Z(IT),NROW)
      GO TO 149
  148 CALL GMMATS (Z(IB),NROW,NROW,0,Z(IMH),NROW,NROW,0,Z(IV))
