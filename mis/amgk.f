...
C
      ...
      INTEGER         N95AEO,N95AEG,IHIT(16),NHIT,IAECV,N95DLQ
      INTEGER*8       KBASE,KEYP(16),NBY,NDT,NZ8
      REAL            RFKV(NB)
...
C
   ...
   10 CONTINUE
C
C     THE ELEMENTS, ALL PAIRS AT ONCE - OR, WITH N95_AERO_CACHE SET,
C     FROM THE CACHE WHEN IT HOLDS EVERY PAIR OF THE BATCH (MSC/
C     MSCAEC.C). AN ENTRY IS KEYED BY A HASH OF EVERY INPUT THE
C     ELEMENTS DEPEND ON - THE RECORD JUST READ AND ITS FOUR COUNTS, NJ,
C     THE SYMMETRY FLAGS, THE REFERENCE CHORD, THE MACH NUMBER AND THE
C     PAIR'S REDUCED FREQUENCY - AND HOLDS THE BITS GENDK COMPUTES. THE
C     PAIRS IT LACKED ARE STORED AFTER GENDK. IAECV NAMES THE KERNEL (1
C     NASA'S PARABOLIC, 3 THE QUARTIC OF MIS/DLMQ.F): A CHANGE TO WHAT
C     GENDK COMPUTES MUST CHANGE IT.
C
      NHIT = 0
      IF (N95AEO() .EQ. 0) GO TO 12
      IAECV = 1
      IF (N95DLQ() .NE. 0) IAECV = 3
      KBASE = 0
      NBY   = 4
      CALL N95AEH (IAECV,NBY,KBASE)
      CALL N95AEH (NJ,NBY,KBASE)
      CALL N95AEH (ND,NBY,KBASE)
      CALL N95AEH (NE,NBY,KBASE)
      CALL N95AEH (REFC,NBY,KBASE)
      CALL N95AEH (FMACH,NBY,KBASE)
      NBY   = 16
      CALL N95AEH (NP,NBY,KBASE)
      NBY   = 4*INT(NREAD,8)
      CALL N95AEH (WORK,NBY,KBASE)
      NDT   = 8*INT(NTPS,8)*INT(NTPS,8)
      NZ8   = 0
      NBY   = 4
      DO 11 P = 1,NB
      KEYP(P) = KBASE
      CALL N95AEH (RFKV(P),NBY,KEYP(P))
      IHIT(P) = N95AEG(KEYP(P),1,DTB(1,1,P),NDT,DTB(1,1,P),NZ8)
      CALL N95AES (1,IHIT(P))
      NHIT = NHIT + IHIT(P)
   11 CONTINUE
   12 IF (NHIT .EQ. NB) GO TO 14
      CALL GENDK (WORK(INC),WORK(INB),WORK(IYS),WORK(IZS),WORK(ISG),
     1            WORK(ICG),WORK,NB,FMACH,RFKV,DTB,NTPS)
      IF (N95AEO() .EQ. 0) GO TO 14
      DO 13 P = 1,NB
      IF (IHIT(P) .EQ. 0) CALL N95AEP (KEYP(P),1,DTB(1,1,P),NDT,
     1                                   DTB(1,1,P),NZ8)
   13 CONTINUE
   14 CONTINUE
C
...
C
     ...
     6             DIIJI(16)
C
C     THE QUARTIC KERNEL (NASTRAN SYSTEM(270)=1): INCROQ FOR THE BATCH,
C     AS INCRO HANDS OVER TO IT (MIS/DLMQ.F)
C
      IF (N95DLQ() .EQ. 0) GO TO 3
      CALL INCROQ (AX,AY,AZ,AX1,AY1,AZ1,AX2,AY2,AZ2,SGR,CGR,SGS,CGS,
     1             NB,KRV,FL,BETA,SDELX,DELY,DELR,DELI)
      RETURN
    3 M     = SQRT(1.0 - BETA**2)
      BR    = FL/2.
      EPS   = 0.00001
