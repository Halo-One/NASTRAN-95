...
C
  ...
  800 IN1 = IN1 + CBAR
      ...
      IN1 = IN2 + BBBAR
      IN2 = IN2 + KKK
C     HALO: LOOP 810 AS ONE OPTIMISED CALL (N95DOT, MIS/N95TWIN.F) WHEN
C     IT RUNS TO BBBAR1 AND DX(IN1) IS NONE OF ITS TERMS - THE SAME
C     PRODUCTS SUBTRACTED IN THE SAME ORDER; ELSE THE LOOP ITSELF
      NIT = BBBAR1 - KKK
      IF (NIT .LT. 1) GO TO 810
      IF (IN1.GT.IN2 .AND. IN1.LE.IN2+NIT) GO TO 810
      IF (IN1.GT.IN3 .AND. IN1.LE.IN3+NIT) GO TO 810
      CALL N95DOT (DX(IN1),DX(IN2+1),DX(IN3+1),NIT,-1)
      IN2 = IN2 + NIT
      IN3 = IN3 + NIT
      KKK = BBBAR1
      GO TO 790
