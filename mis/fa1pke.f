...
C
C     HALO: THE ROOT'S EIGENVECTOR, FOR THE TRACKING AT THE END OF THE
C     LOOP (FA1PKU); THE MATRICES ARE THOSE OF THE REDUCED FREQUENCY
C     THE ROOT CONVERGED AT
C
  210 CALL FA1PKT (Z(IMA),Z(IV),Z(IB),NROW,Z(K),NROOT,Z(BUF1))
      NROOT  = NROOT + 1
C
...
C
C
C     HALO: THE LOOP'S ROOTS RE-ORDERED TO FOLLOW THE PREVIOUS LOOP'S
C     (EIGENVECTOR CORRELATION AND EIGENVALUE DISTANCE), SO THAT POINT N
C     OF THE FLUTTER SUMMARY IS ONE ROOT THROUGHOUT
C
  300 I = (FLOOP-1)*3
      CALL FA1PKU (Z(IEIGNS+1),NROOT,NROW,Z(IT),IP0-IT,MXHH,Z(BUF1),
     1             FLOOP,Z(IMVR+I))
      IF (ISTART .NE. 0) GO TO 310
      ISTART = 1
