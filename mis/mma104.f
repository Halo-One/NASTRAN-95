...
4000  CONTINUE
     ...
     & .AND. ZD( IBROW2+1) .EQ. 0.D0 ) GO TO 4500
      ...
      IROWAN = ZI( INDXA-1 )
      INDXA  = ( ( INDXA+1 ) / 2 ) - 2  
C HALO: THE LOOP BELOW, AS A CALL TO AN OPTIMISED COPY OF ITSELF (MMA10X,
C HALO:   MIS/MMA10K.F); ITS STATEMENTS KEPT FOR READING
      CALL MMA10X ( ZDC(IDX4+IROWA1), ZD(INDXA+2), ZD(IBROW2),
     &              IROWAN-IROWA1+1 )
      GO TO 4500
...
C HALO: THE LOOP BELOW, AS A CALL TO AN OPTIMISED COPY OF ITSELF (MMA10D,
C HALO:   MIS/MMA10K.F); ITS STATEMENTS KEPT FOR READING
      CALL MMA10D ( ZDC(IDX4X+I), ZD(INDXA+1), ZD(INDXB+1),
     &              IROWN-IROW1+1 )
      GO TO 45000
      DO 44000 K = 1, KCNT, 2
