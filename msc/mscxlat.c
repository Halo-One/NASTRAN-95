...
#include <windows.h>
#define RS_SEP "\\"
#define rs_stricmp _stricmp
#else
/* HALO: the POSIX spellings of the restart helpers below: realpath for
 *   _fullpath (msc_abs_path, mscmain.c), stat for the directory test,
 *   strcasecmp, '/' between a directory and a file, and link(2) with a
 *   copy as the fallback for CreateHardLink / CopyFile              */
#include <strings.h>
#include <sys/stat.h>
#include <unistd.h>
#define RS_SEP "/"
#define rs_stricmp strcasecmp
#endif
...
int msc_restart_set(const char *modes_deck, const char *optp)
{
    ...
    const char *p, *name;
    FILE *f;
    msc_abs_path(full, sizeof(full), modes_deck);
    ...
    if (optp && optp[0] && rs_stricmp(optp, "none") != 0) {
        int is_dir;
#ifdef _WIN32
        DWORD a;
#else
        struct stat sb;
#endif
        msc_abs_path(where, sizeof(where), optp);
#ifdef _WIN32
        a = GetFileAttributesA(where);
        is_dir = a != INVALID_FILE_ATTRIBUTES && (a & FILE_ATTRIBUTE_DIRECTORY);
#else
        is_dir = stat(where, &sb) == 0 && S_ISDIR(sb.st_mode);
#endif
        if (is_dir) {
            snprintf(g_restart_dic,  sizeof(g_restart_dic),  "%s" RS_SEP "%s.dic",  where, name);
            snprintf(g_restart_nptp, sizeof(g_restart_nptp), "%s" RS_SEP "%s.nptp", where, name);
            snprintf(g_restart_prt,  sizeof(g_restart_prt),  "%s" RS_SEP "%s.out",  where, name);
        } else {
            char ws[MSC_PATHLEN];
...
int msc_restart_link(const char *outdir)
{
    ...
    snprintf(target, sizeof(target), "%s" RS_SEP "optp.nptp", outdir);
#ifdef _WIN32
    DeleteFileA(target);
...
#else
    unlink(target);
    if (link(g_restart_nptp, target) == 0) return 0;
    {
        /* another file system: a copy */
        FILE  *from = fopen(g_restart_nptp, "rb"), *to = NULL;
        char   buf[65536];
        size_t nb;
        int    ok = 0;
        if (from) to = fopen(target, "wb");
        if (from && to) {
            ok = 1;
            while ((nb = fread(buf, 1, sizeof(buf), from)) > 0)
                if (fwrite(buf, 1, nb, to) != nb) { ok = 0; break; }
        }
        if (from) fclose(from);
        if (to && fclose(to) != 0) ok = 0;
        if (ok) return 0;
    }
#endif
