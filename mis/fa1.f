...
C
     ...
     2                BXHH,SCR1,REC0(9),FLUT(10),IMETH(2),FMETHD,SMETH,
     3                TRL(10),AERO(2),FLFACT(2),FLUTER(2),
     4                SR,SM,SK,PR,PM,PK,SL,IBLOCK(12),METHOD(5)
      ...
      LOGICAL         PKNL
      COMMON /BLANK / FLOOP,TSTART,ICEAD,PKMATC
     ...
     1,   FLIST /106/, FSAVE /201/, KXHH /202/, BXHH /203/,   MXHH /204/
      ...
      DATA  IMETH   /  4HS   ,4HL   /
      DATA  NMD /5  /, METHOD/4HK   ,4HKE  ,4HPK  ,4HINV ,4HPKNL/
...
C
   ...
   40 CONTINUE
      GO TO 490
   50 PKNL = .FALSE.
      IF (I .NE. 5 .AND. .NOT. (I .EQ. 3 .AND. PKMATC .NE. 0)) GO TO 51
C
C     HALO: A MATCHED POINT ANALYSIS - THE DENSITY, MACH AND VELOCITY
C     LISTS WALKED TOGETHER, ENTRY BY ENTRY, INSTEAD OF EVERY COMBINATION
C     (MSC'S PKNL). ASKED FOR BY PARAM PKMATCH 1 WITH METHOD PK (THE
C     RIGID FORMAT PASSES THE PARAMETER), OR BY THE METHOD SPELLED PKNL.
C     IT RUNS AS PK; ONLY THE FLUTTER LOOP LIST BELOW KNOWS THE DIFFERENCE.
C
      PKNL = .TRUE.
      I = 3
   51 REC0(3) = I
...
C
C     HALO: A NINTH HEADER WORD TELLS FA2 THE METHOD WAS PKNL, SO THAT
C     IT PRINTS THE MATCHED POINTS AS ONE BLOCK PER ROOT
C
      REC0(9) = 0
      IF (PKNL) REC0(9) = 1
      CALL WRITE (FSAVE,REC0,9,1)
      BREF = REC(6)
      RREF = REC(7)
      NEIW = REC0(8)
C
...
C
   ...
   97 NLOOPS = 0
      ...
      IF (.NOT. PKNL) GO TO 100
C
C     HALO: PKNL - ONE LOOP PER ENTRY OF THE DENSITY AND VELOCITY LISTS
C     TAKEN TOGETHER, THE MACH LIST WALKED ALONGSIDE (ITS LAST ENTRY
C     REPEATED WHEN IT IS SHORTER), INSTEAD OF EVERY COMBINATION
C
      PM = SM
      PR = SR
      PK = SK
   98 PR = PR + 1
      PK = PK + 1
      IF (IZ(PR).EQ.-1 .OR. IZ(PK).EQ.-1) GO TO 130
      IF (IZ(PM+1) .NE. -1) PM = PM + 1
      DLT(1) = Z(PM)
      DLT(3) = Z(PR)
      DLT(2) = Z(PK)
      NLOOPS = NLOOPS + 1
