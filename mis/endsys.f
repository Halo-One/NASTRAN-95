...
C
C HALO: LINK added to the EXTERNAL list. gfortran provides LINK as a GNU
C HALO:   intrinsic subroutine wrapping the link(2) system call, so an
C HALO:   undeclared CALL LINK below binds to that instead of to NASTRAN own
C HALO:   overlay-switch routine and fails on the argument count.
