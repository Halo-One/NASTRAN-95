...
C
   ...
   90 CONTINUE
C
C     HALO: THE LOOP BELOW IN CORE, THE PAIRS SOLVED SIDE BY SIDE, WHEN
C     AMPK CAN (MIS/AMPK.F SAYS WHEN, AND WHY THE ANSWER IS THE SAME)
C
      CALL AMPK (AJJL,SKJ,SCR4,SCR5,SCR6,SCR3,IMAX,IDONE,NLEFT)
      IF (IDONE .EQ. 0) GO TO 95
      IF (NLEFT .GT. 0) CALL MESAGE (45,NLEFT,NAME)
      GO TO 110
   95 CONTINUE
C
C     LOOP ON MK PAIRS
C
C     HALO: NO FILE POSITION CARRIED INTO THE LOOP (SEE AMPC, AMPD)
C
      NEXTC=0
      NEXTS=0
      XKO=-1.0
